# Week 1 — Release notes & lessons learned (2026-04-28)

Completed the Week 1 exercises in this folder for CS146S-style prompting practice (local Ollama, `mistral-nemo:12b` / `llama3.1:8b` per assignment scripts).

### Highlights

- **K-shot prompting** (`k_shot_prompting.py`): tuned few-shot system prompt for constrained string output.
- **Chain-of-thought** (`chain_of_thought.py`): system prompt requiring explicit modular-arithmetic reasoning and final `Answer:` line.
- **Tool calling** (`tool_calling.py`): system prompt instructing JSON tool calls matching the in-repo executor schema.
- **Self-consistency** (`self_consistency_prompting.py`): system prompt structured for stable majority voting under high temperature.
- **RAG** (`rag.py`): wired `YOUR_CONTEXT_PROVIDER` to supply `data/api_docs.txt` so generation is grounded in retrieved context.
- **Reflexion** (`reflexion.py`): added reflexion system prompt and user-side context builder (prior code + test failures) for a single repair pass.

### Tooling (repo root)

- **Poetry** (`../poetry.toml`): in-project virtualenv preference (`in-project = true`); `.venv` remains gitignored.
...
- Assignment overview: [`assignment.md`](./assignment.md)
- Course / repo context: [`../README.md`](../README.md)
